...
- [ ] 0.3 **git 历史处置（仅用户可决，re-public 的前置）**：仓库已转 private（2026-06-11 用户执行）✅ → 下一步二选一：A. `git filter-repo` 重写历史 + force push；B. 以脱敏后内容重建新仓库（更简单干净，但丢失 commit 历史叙事——而"全程留痕"是旗舰文卖点，需权衡）
