...
本模式的 v0.1 框架实现位于 `kb_forge/invariant_scaffold/`（Sprint O Stage 1 首次抽象 / dogfood PASSED 11/11 + 4/4）：
...
跨领域案例方应**先复制 kb_forge/invariant_scaffold/ 改填**，再回过头读本文件理解设计哲学。

| 本文件章节 | kb_forge/invariant_scaffold/ 对应实现 |
|---------|--------------------------------|
...
### 8.6 详细实证 + 反模式（v0.2.1 fold / Sprint AA 批 2 / per Sprint X 方案 B）

#### 8.6.1 Sprint O 4/4 catch 详细实证

per Sprint O Stage 1.13 dogfood：4 SelfTest impl 通过 SelfTestRunner 注入违反 / 4/4 catch。

**典型 SelfTest impl**（V9 lower-bound self-test 简化示例）：

```python
class V9LowerBoundSelfTest:
    name = "v9_lower_bound_self_test"
    target_invariant = "v9"

    async def setup_violation(self, conn) -> None:
        # transaction 内：插入 active person 但不给 primary name
        await conn.execute("""
            INSERT INTO persons (id, slug, deleted_at)
            VALUES ('test-self-test-uuid', 'test-self-test', NULL);
        """)
        # （故意不插对应 person_names primary 行 / V9 应 catch）
```

`SelfTestRunner` flow：
1. `BEGIN`
2. `setup_violation(conn)` → 插入 1 行违反数据
3. 跑 V9 query → assert violations 包含 `test-self-test-uuid` ✓
4. `ROLLBACK` → production data 不受影响

**4/4 catch 数据点**（Sprint O Stage 1.13）：
- V9 lower-bound: 1/1 catch ✓
- V8 containment: 1/1 catch ✓
- V10.a orphan: 1/1 catch ✓
- V10.b orphan: 1/1 catch ✓

详见 `docs/sprint-logs/sprint-o/stage-1-dogfood-2026-04-30.md`。

#### 8.6.2 Self-Test 反模式

❌ **无 transaction rollback** — setup_violation 直接 commit / 污染 production data / 必须用 transaction wrap

❌ **query 与 invariant 共享模板** — self-test 写自己的 query verify catch / 不复用 invariant query（否则 query 写错时 self-test 也会同时错 / 失去防 false-pass 价值）

❌ **仅断言 row count** — `assert violations > 0` 不够 / 应 assert `test-self-test-uuid in violations`（确认 catch 的就是注入的 / 不是别的真实 violation 巧合命中）

❌ **self-test 仅在 dev / 不进 CI** — self-test 应每次 sprint 收口跑（per §3.3 实现模板 + Stage 1.13 sanity gate）/ 仅 dev 跑 = 仅有"写过 self-test"的形式 / 失去持续防御价值
...
Sprint O 把 services/pipeline/tests/test_invariants_*.py + qc/* 的 11 invariants 抽象到 kb_forge/invariant_scaffold/（Python → Python，**同 stack**）。但与 Sprint N identity_resolver 的 byte-identical 不同，invariant_scaffold 走 **soft-equivalent + self-test 注入** 双 dogfood：

- production query: 11 invariants 直接跑 services/pipeline 实现 → 0 violations
- framework path: 11 invariants 跑 kb_forge/invariant_scaffold + examples/huadian_classics adapter → 0 violations
...
| Q (audit_triage soft-equivalent / 跨 stack) | 64 pending / 字段一致 | `kb_forge/audit_triage/examples/huadian_classics/test_soft_equivalent.py` |
...
| Draft v0.1.1 | 2026-04-30 | 首席架构师 | Sprint O Stage 1 cross-reference §7 加（紧密化 kb_forge/invariant_scaffold/）|
...
| **v0.2.1** | **2026-04-30** | **首席架构师** | **Sprint AA 批 2 fold (T-X02-FW-001 / per Sprint X 方案 B deferred)：fold §8.6 详细实证 + 反模式（§8.6.1 Sprint O 4/4 catch 详细实证含 SelfTest impl 范例 + SelfTestRunner flow / §8.6.2 Self-Test 反模式 4 条）；方案 B 紧凑写法（实证 1 例 + 反模式 4 条不展开 / 控制体量）** |
...
> **Sprint AA §8.6 fold deferred 内容（v0.2.1）**：4/4 实证 + 反模式 first-class / 完整 §8 Self-Test Pattern 闭环。
