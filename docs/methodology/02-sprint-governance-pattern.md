...
## 9. Sprint 形态与 5-Stage 模板的关系（v0.2 重述 / Sprint V 批 4 加 §14+§15）

§2 给出的 5-stage 是**数据管线类 sprint** 的标准模板（实证：Sprint A-K）。但实际 AKE 项目中存在多种 sprint 形态，并不都需要 5 stage。下面 6 类（§10-§15）是华典智谱 Sprint L-V 实证沉淀的**元 pattern**——它们告诉你：什么时候用精简模板 / 什么时候触发暗规则升级 / 什么时候宣布"抽象阶段终点" / 怎么跨 stack 抽象 / 什么时候做评估 sprint / 什么时候做 release sprint。

| 元 pattern | 适用 | 实证 sprint | 详见 |
|-----------|------|-----------|------|
| Maintenance Sprint Pattern | 5-batch 清债 sprint | Sprint P / R / V | §10 |
| P3 复发升级 P2 暗规则 | 跨 sprint 同 bug 复发 ≥ 2 次 | DGF-N-02 → DGF-O-01 (Sprint P) | §11 |
| 5 模块齐备阈值 | framework 抽象阶段终点判断 | Sprint Q (5 模块齐备 ⭐) | §12 |
| 跨 stack 抽象 pattern | 生产 stack ≠ framework stack | Sprint Q audit_triage + Sprint T Docker dogfood | §13 |
| **Eval Sprint Pattern** ⭐ | release / 成熟度评估决策（不立即触发动作）| Sprint S (v0.3 release timing) + Sprint U (v1.0 评估) | **§14**（v0.2 新增）|
| **Release Sprint Pattern** ⭐ | release 触发条件达成 → 启动 release sprint | Sprint P (v0.2.0) + Sprint T (v0.3.0)| **§15**（v0.2 新增）|

> 6 patterns 之间的关系：
> - §10 (Maintenance) + §14 (Eval) + §15 (Release) 是 **3 种 framework v0.x 演化期 sprint 形态**（vs §2 5-stage 数据管线 sprint）
> - §11 (P3 复发升级) + §12 (5 模块齐备) 是 **跨 sprint 暗规则**（影响所有 sprint 形态的决策）
> - §13 (跨 stack 抽象) 是 **抽象工程模式**（与具体 sprint 形态无关 / 适用于任何抽象 sprint）
...
```
Sprint L → kb_forge/sprint-templates/        (governance ① )
Sprint M → kb_forge/role-templates/          (governance ② )
Sprint N → kb_forge/identity_resolver/       (code ① )
Sprint O → kb_forge/invariant_scaffold/      (code ② )
Sprint Q → kb_forge/audit_triage/            (code ③ ) ← 5 齐备 ⭐
```
...
❌ "再开个 kb_forge/llm_gateway/ / kb_forge/cost_tracker/ 模块" — 这些是案例方的事，不是框架的事
...
- framework：`kb_forge/audit_triage/` （Python / asyncpg / 14 files / 2553 行）
...
## 14. Eval Sprint Pattern（评估 sprint 形态 / v0.2 新增 / Sprint V 批 4）

### 14.1 触发

**评估 sprint** 是 framework v0.x 演化中的 first-class 形态——专门用来做"决策但不立即触发动作"的工作（vs Maintenance / Release / 抽象 sprint 都直接触发实施）。

3 类典型触发：

1. **Release timing 评估**：累计 v0.x 候选成熟 → 评估是否到 release 时机（实证：Sprint S v0.3 release timing → ADR-030）
2. **成熟度评估**：≥ 5 sprint zero-trigger → 评估更高 milestone 候选议程（实证：Sprint U v1.0 候选议程评估 → ADR-031）
3. **方向调整评估**：跨 sprint 累积发现需调整 framework / methodology 方向（vs 单 sprint patch 解决）

### 14.2 1.5-2 会话标准结构（实证：Sprint S / 1.5 会话 + Sprint U / 1.5 会话）

```
批 1 — 主 ADR 起草（trigger 或 eval）       ~30-45 min
批 2-N — methodology v0.x cross-ref polish   ~25 min × N
        + 实证锚点段（per /01 v0.1.2 / /03 v0.1.2 / /04 v0.1.2 模式）
Stage 1.13 — sanity 回归（不动 code / 简单）  ~10 min
Stage 4 — closeout + retro                   ~30 min
total: 1-2 sessions
```

### 14.3 与 §10 Maintenance / §15 Release 的差异

| 维度 | §10 Maintenance | **§14 Eval** ⭐ | §15 Release |
|------|----------------|---------------|-------------|
| 触发 | v0.x 候选 ≥ 5 累积 | 决策需求（release timing / 成熟度 / 方向）| release 触发条件达成 |
| 主要输出 | patch + small docs | **ADR + methodology cross-ref polish** | RELEASE_NOTES + version bump + tag |
| 是否 land 新 patch | ✅ | ❌（评估 sprint 不 land 新 patch / 仅 docs）| ✅（fold 押后 candidates）|
| 是否触发 version bump | 否（patch sprint）| **否**（评估 sprint 仅 ADR + docs）| ✅（v0.x → v0.(x+1)）|
| typical 工时 | 1 会话 | **1.5-2 会话** | 1-2 会话 |
...
|------|---------------------------------|-------------------------------|
| 决策性质 | 触发条件 ≥ 阈值 → **立即 release** | 触发条件 < 阈值 → **不立即 release / 锁定条件** |
...
❌ **Maintenance sprint 中顺手起 ADR**——破坏 maintenance zero-ADR 节奏 / 应该拆 separate eval sprint

❌ **评估 sprint 内 fold 大 feature**——破坏评估纯净性 / 应该让 feature land 进 maintenance / release sprint

❌ **跳过评估直接 release / 立即升 milestone**——错过"锁定条件 + 路径预测"的价值（per Sprint U ADR-031 实证 / 评估 sprint 让 v1.0 路径明确：2026-10~2027-04）

✅ **评估 sprint 后**应该有 1+ 个 maintenance / release sprint 跟进（Sprint S → Sprint T release / Sprint U → Sprint V maintenance）

---

## 15. Release Sprint Pattern（release sprint 形态 / v0.2 新增 / Sprint V 批 4）

### 15.1 触发

release 触发条件全达成（per release-trigger ADR / e.g. ADR-030 §2 6/6 触发条件 → Sprint T = v0.3 release sprint）。

### 15.2 5-batch 标准结构（实证：Sprint P v0.2.0 release / Sprint T v0.3.0 release / Hybrid 含大 feature fold）

```
批 1 — 大 feature fold（Hybrid 形态 / 如有）  ~3-4h（非必须）
批 2 — N 模块 README §0+§8 + version bump   ~30 min
批 3 — kb_forge/RELEASE_NOTES_v0.X.md       ~45 min
批 4 — STATUS / CHANGELOG release 标记       ~20 min
批 5 — sanity + dogfood + ADR §5 回填        ~30 min
Stage 4 — closeout + retro                   ~30 min
total: 1-2 sessions（不含批 1 大 feature 时 / 含批 1 时 2-3 sessions）
```

### 15.3 Hybrid 形态（release + 大 feature fold）

per Sprint T T-V03-FW-005 实证：可在 release sprint 内 fold 1 个押后大 feature（如 Docker compose dogfood infra）。

**Hybrid 适用条件**：
- 押后 feature ≥ 4h 工作量（vs ≤ 1h fold 进 maintenance batch）
- feature 与 release 主题相关（如 Sprint T Docker dogfood 是 v0.3 dogfood infra 升级）
- release 不急（多花 1 sprint fold 不阻塞 timeline）

**Hybrid 反模式**：
- ❌ 在 release sprint 内 fold > 1 个大 feature（scope creep）
- ❌ 在 release sprint 内 fold 不相关 feature（应该单独 mini-sprint）
...
```
- [x] release sprint 工时 ≤ 预算
- [x] 0 Stop Rule 触发
- [x] git tag pushed
- [x] RELEASE_NOTES_v0.X.md 落地
- [x] N 模块 statement 一致（统一版本号）
- [ ] 跨外部读者 review（待外部触发）
```

回填即让 release-trigger ADR 完整闭环（vs 仅起草而不回填）。

### 15.5 反模式

❌ **release sprint 中临时改触发条件**——破坏 ADR 锁定（应该走 ADR addendum / 不能 inline 改）

❌ **跳过 sanity + dogfood gate**（per 批 5 必做）——可能引入回归而 release tag 已 push（不可逆）

❌ **不打 git tag 就完成 sprint**——release sprint 没有 tag = 没 release（实证：Sprint P / T 都 push tag）

✅ release sprint 后通常**至少 1 个 maintenance sprint**（避免立即 release 后再 release / 让 release 节奏稳定）
