...
## 2026-07-17 (外联) — 外审信 C/B 发出（真相表 ❌ 首次清零）+ 外联套件转私

### [docs] 真相表外审 ❌→🟡 + outreach-kit 转私（用户隐私决策）

- **用户执行**：外审信 C（会计/审计方向）+ B（STS 方向）2 封冷邮件已发出（架构师填好收件人与锚定论文的成品邮件 / 对象身份仅存用户邮箱）。真相表「外审对接」❌→🟡（判据锚=收到实质回复，audit #10 机制）；叠加并行会话同日更新（第 5 篇 dev.to 发表 5/4 ✅ + baojie YAML 独立技术收敛 引用讨论 ❌→🟡）→ **11 项判据 ❌ 首次清零（✅7/🟡4/❌0）**。克制读法：两个原 ❌ 仅升至"有动作无结果"的 🟡
- **外联套件转私（用户决策）**：`docs/reports/outreach-kit-2026-07.md`（信件模板全文 + 找人配方 + 追踪表）从公开仓移除——收件人可反向搜索信件原句定位本仓，暴露"谁在被联系、按什么剧本"。处置：完整套件移入 `docs/reports/private/`（新增 gitignore 规则 / 永不提交），公开位置留转私说明存根；追踪表改**代号制**（C-1/B-1/A-1，真名任何情况不落盘——比照 ADR-040 映射不落盘纪律）。历史注：模板早期版本仍在 git 历史（GitHub 代码搜索仅索引 HEAD，主要反查面已消除；彻底清除可并入下次 filter-repo 批次，评估为非紧急）
- **同批更正**（用户补报）：q58413890 知乎回答实为 2026-06-21 发布（见 6/21 回填条目内注）——audit"成稿 25 天未发布"更正为记录缺失
- **7/25 云端回收任务已同步更新**：信已发出口径 + 知乎回答互动数纳入 A/B 对照
