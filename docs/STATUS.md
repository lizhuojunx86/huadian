...
- **最近更新**：2026-07-11（**外部动作执行包 + 2026-07 月度日记 v0.1**：① 2026-07 月度日记 v0.1 起草（Q5 节律第 3 篇 / 覆盖 6/2→7/11 / 核心元判断：瓶颈第三次转移 写作产能→审稿消化→**用户侧外部真人动作** / 待用户审稿定版）；② 蒲公英发帖包 [articles/publish/ouryao-data-integrity-post-ready.md](articles/publish/ouryao-data-integrity-post-ready.md)（Discuz 纯文本适配复制即发 + 回帖预案 4 情形含闸 2 模板）；③ 08-EN 预印收口：2 处 [AUTHOR TO SUPPLY] 补齐（通讯 + AI 协作声明 + data availability 指向 public repo）+ **SSRN 上传用 PDF 已生成**（14 页 / pandoc+xelatex / mermaid→文本树）；④ 外审执行包 [reports/outreach-kit-2026-07.md](reports/outreach-kit-2026-07.md)（3+1 封成品信 C 会计·B STS·A 药企QA·B' 合作者 + 每类 10 分钟找人配方 + 追踪表）；⑤ 单页行动清单 [reports/user-actions-2026-07-11.md](reports/user-actions-2026-07-11.md)（蒲公英→SSRN→外审→commit 四步）；⑥ **用户当日执行：SSRN 预印提交完成**（abstract_id **7099138** / https://ssrn.com/abstract=7099138 / 作者 Zhuojun Li / Independent / staff review 1-5 工作日 / **case-2 首个公开学术制品** / 链接已回填外审信件——三封信只剩 2 占位）；⑦ **用户当日执行：蒲公英发帖完成**（https://www.ouryao.com/thread-816324-1-1.html / 商业探针闸 1 启动 / ~7/25 回收信号）+ **外审信时序修正**：等 SSRN 审核通过后再发（每日 09:05 自动检查任务 `ssrn-7099138-review-watch` 已设，通过即提醒）。**定位：真相表仅剩 2 ❌（引用讨论/外审对接）均需用户真人动作，本批把摩擦降到"打开→复制→发送"级**。commit 待用户本地（沙箱 .git 只读）⭐）/ 前序：2026-06-20（**公开输出第 2 批 + L1 真相核查 + case-2 英文预印 + 对接 SOP**：① 知乎 3 篇（6-13）+ 掘金镜像 3 篇（6-19）+ **第 4 篇「LLM 实体不应自动合并」已发（6-20）→ 真相表「发表数」🟡3/4→✅4/4**；② shiji-kb（2112★）GitHub Discussion 用户已发出搭话 → 「近邻交流」❌→🟡；③ 信号回收：知乎每篇阅读数十/收藏点赞个位数/无评论 → 「引用讨论」诚实保持 ❌；④ **B3 L1 解冻调研：实测领域无关比例 64.4%（core+tests），但 core 本身 0 领域字样——「比例偏低」是单一 example 过重的度量假象、非泄漏，故原"下沉泄漏"方案前提不成立 → 产出 [l1-domain-agnostic-finding](reports/l1-domain-agnostic-finding-2026-06-20.md)，真正杠杆=import-root 改名 或 2nd domain → **用户选 ①+③：③判据重定义已落地（真相表领域无关 ❌→🟡）；①改名已执行（`framework/`→`kb_forge/` / 73 imports / pytest 93 passed / ADR-039）→ 代码推进 🟡→✅**；⑤ case-2 08 英文 working paper 起草（SSRN 用 [preprints/08-EN](articles/preprints/08-dual-reverse-calculation-pattern-EN.md)）；⑥ 外审/学术合作者对接 SOP 产出；⑦ 商业试探：08/10 数据完整性检测 → 药企 QA/监管市场的论坛试水稿（蒲公英）。**commit 待用户本地（沙箱 .git 只读）** ⭐⭐）/ 前序：2026-06-06（**case-2 / GO-AC-η：三篇目标投放渠道候选完成（review §5 第 4 项 / author-side 收口）**。research-first web 核实各渠道当前 scope+截稿窗 → 用户定**双轨（中文域内《中成药》/《中国医药工业杂志》+ 英文方法论 STS/会计/AI）+ 预印先行（arXiv/SSRN）**。shortlist：08→AOS/JBE/FAccT2027 · 09→SSS/ST&HV/ESTS/BD&S · 10→PDA/法务会计/中文域内。时序：FAccT/AIES 2026 窗已过→2027 周期；期刊滚动→v1.0(2026-10)后投；预印即时。产出 `docs/methodology/case-2-target-venues-v0.1.md`。**里程碑：三篇 v0.4 四项 author-side（citation / n=2 推断边界 / 识别≠证实 / 目标渠道）全部完成**（GO-AA/AB/AC-η）；余外部审稿（2026-09 错峰 09→08→10）+ 翻译工序（v0.4 后）。**commit 待用户本地（沙箱 .git 只读）** ⭐⭐）/ 前序：2026-06-05（**case-2 / GO-AB-η：09+10 v0.4 审稿前置批（镜像 08 / author-side）完成**。把 08 已批 canonical 镜像到 09·10：09 §11 n=2「推断边界」+「识别≠证实」canonical + §12 全 15 条卷页 web 最终化（5 条新核实 Knorr Cetina·Perrow·Van Maanen·Vaughan·Studdert / Goodhart·三书套 08 同款）；10 §10「单企业数据」前缀「推断边界（n=2）」+ **新增「识别≠证实」条**（10 原无此条）+ §11 按 fork 保留编号+注释·仅标点对齐 08/09。三篇 n=2 / 识别≠证实 / citation 全绿。配套：case-2 §10.32 + review §5 三项全绿 + 09·10 footer。不改论点；09·10 **不 bump v0.4**。**commit 待用户本地（沙箱 .git 只读）** ⭐⭐）/ 前序同日：2026-06-05（**case-2 / GO-AA-η：08 v0.4 审稿前置批（author-side / 不等外部审稿人）完成**。08 单篇三项 author-side 编辑：①§9.1 标 `[PoC-validated 2026-06-04]` + 并入 **E4 候选（结构化输出持久化）** + 命名·去单位化修正（YAML `mode:"sigma_floor"` 去 F004 / `sigma_floor:0.5` 去 pp / `output_as_dict()` / §5 教学伪代码按用户决策留 pp）；②§10 全 12 条 citation **web 卷页最终化**（Goodhart 补原版 RBA *Papers in Monetary Economics* Vol.I + 权威重印 pp.91–121 / Macmillan 1984 / Springer 元数据确认；Latour·Wenger·Scott 补出版地+副标题；8 篇期刊卷期页码逐条复核**无误**）；③§8 `n=2「推断边界」`+`「识别≠证实」方法论自觉边界` **canonical 化**（仅定 08 / 09·10 后续批镜像 / 遵 09→08→10 错峰）。配套：case-2 §10.31 + review §2.1·§5 标 08 完成 + 08 Status 行·footer 记前置批。不改论点；08 **不 bump v0.4**（外部审稿未启 / 仍 v0.3 工作稿 + 前置批标注）。**commit 待用户本地（沙箱 .git 只读）** ⭐⭐）/ 前序：2026-06-04（**case-2 / GO-Z-η：traceguard E1-E3 真实 PoC 完成（Layer 1 框架 / case→框架）**。08 §9.1 文章内设计 → traceguard 真实可跑代码：E2 check 注册器 + E1 generic `reverse_calc`（σ-floor+edge-band / 标量从 output_preview 回取）+ E3 audit-flag（`EvalTrace.flag_type` + 幂等 forward-only `ensure_schema` + `get_step_stats` 把 suspicion 排除出 `pass_rate`/`avg_score`）。真 6 批 σ 矩阵经真实 CLI `guardian check` 端到端 fire（`action=alert` / σ=0.0203 贴 40.0 下限 / pass_rate 不污染=非 5/6）+ 自然方差负对照不 fire + DEGRADED 无 LLM 照跑 / dogfood **246 passed**（224 基线 +22 新 / 0 回归 / ruff 零新增）。**新暴露 E4 候选（结构化输出持久化）= 纯设计稿发现不了的框架抽象产出 / 2027-01 框架 v0.1 候选**。generic 进 core / 中药进 `configs/examples/tcm_extraction.yaml`（第 2 个 domain config）。回填：§10.30 + v0.4 review §2.1 fold（08 §9.1 标注挂 v0.4 批次）。**commit：traceguard b9c1767+a683696+1a341f5 / huadian c5defd3 均已落地（用户本地）；本条 STATUS+CHANGELOG 待本地 commit** ⭐⭐⭐）/ 前序：2026-06-04（**case-2 外部文献 fit 评估登记**：《中药前处理及提取生产质量管理合规性导论》（纸质 ~300 页 / 未到手 / 仅基于介绍文案）→ external-refs 评估文档 v0.1（93 行）+ strategy §10.29。核心判断：该书 = F-001「表层 GMP 合规叙事」权威范本 / 与 case-2 镜像 / **定位价值 > 内容价值**（标定 negative space 边界）。含 12 问题×F-001~F-004 映射 + 三重价值 + 两种污染警告 + **F-005 候选（批均质性 / backlog / 不擅自立）**。仅登记，未启动 OCR / 对照表 / F-005。待本地 commit（沙箱 .git 只读）⭐）/ 前序：2026-06-02（**case-2 / GO-U-η：2026-06 月度日记 v1.0 定版（Q5 节律第 2 篇 / 用户审稿通过 / 全面修订纳入 6/1 GO-O-η~GO-T-η 全程 + 新增 §2.5「节制 vs 爆发」反思 / 165 行）**。前序 6/1 一日 7 阶段：GO-N-η schema v0.4 完整 lock + _meta 0.3→0.4 / GO-O-η~GO-R-η 08 v0.3 精炼版（4015 字）/ GO-S-η~GO-T-η 09 v0.3 精炼版（4054 字）→ **08+09 双文章 v0.3 完整初稿齐**（全量快照各存档）。**GO-V-η（同日续）**：F-003 第 3 篇 `10-subchannel-stratified-detection-pattern` v0.1 outline（197 行 / 12 节 / 招牌「易操纵⊥易暴露」反转悖论 / F-003 从 08 工具升为独立检测分辨率模式 / 三篇三角 09·08·10）。**GO-W-η（同日续）**：10-* v0.1 outline → **v0.2 全本（277 行 / 13 节 / §4 全方差·Simpson 稀释数学 + §5 分层伪代码 + §8 五学术锚点 web 核实 Green&Swets·Simpson·Spence·Benford-Nigrini·IUPAC + §11 参考文献 8 条）**。**GO-X-η（同日续）**：10-* v0.2 → **v0.3 精炼（4474 字 / 删 §12 meta / 全量 v0.2 存 archive 无损 → 三篇全 v0.3 齐平）**。**GO-Y-η（同日续）**：三篇 v0.4 审稿计划 v0.1（视角清单 + 头号风险靶点 + 错峰次序 09→08→10 / planning 非审稿）。**commit：6/1 全程 + GO-U(3)+GO-V(cd493e9)+GO-W(9038822)+GO-X(9b89f7f) 均已落地（用户本地）；GO-Y-η（审稿计划 + §10.28 + STATUS + CHANGELOG）待本地 commit（沙箱 .git 只读）⭐**）/ 历史：2026-05-15 GO-M-ζ（ADR-035 + schema v0.4 outline lock / 沉淀期并行项 D5）/ 2026-04-30 Sprint AA 完成 / Layer 2 维护态首 sprint / methodology/02 + /04 v0.2 → v0.2.1 + brief-template v0.1.5 / 清 3 v0.5 candidates / **连续 12 个 zero-trigger sprint** P→Q→R→S→T→U→V→W→X→Y→Z→AA ⭐⭐⭐ **240% over target** / 1 会话紧凑 / brief-template v0.1.4 第 5 次 dogfood / cross-ref polish 子类首验证 -33% 新发现 / 维护态首 sprint 模板验证
...
- **当前阶段**：**case-2（中成药提取）为活跃主线 — schema v0.1→v0.4 全 lock（3 ADR 033/034/035）+ Layer 2 应用层双文章 08+09 v0.3 完整初稿齐（精炼版各 ~4000 字 / 全量快照存档 / 待 2026-09 v0.4 审稿 → 2026-10 v1.0 双发布 / 领先 ~6-8 周）+ **第 3 篇 10-subchannel-stratified-detection v0.3 精炼（GO-X-η / F-003 / 三篇三角 09·08·10 全 v0.3 齐平）** + Q5 节律 2026-06 月度日记 v1.0 定版**。框架线（Layer 1 / Sprint AB）维护态 D 等候触发。Phase 0 已收尾 / D-route 文档体系全对齐 / methodology v0.2 cycle 完成 8/8 = 100% / **Sprint AA 完成（v0.5 maintenance sprint / 清 T-V05-FW-002 + T-X02-FW-001 + T-X02-FW-002 / methodology/02 + /04 v0.2 → v0.2.1 + brief-template v0.1.5 / sprint-templates v0.3.2 / 1 会话紧凑 / 0 Stop Rule 触发 / Layer 2 维护态首 sprint 模板验证）** / **Sprint AB 默认路径：D 等候触发**（v0.5 maintenance 触发条件未达成 / 候选 ≤ 2 / 阈值 ≥ 3）
...
**一句话**：内部产能（文档 / 治理 / sprint 计数）很高；**对外可验证判据 11 项里 2026-06-20 复核 = ✅ 6、🟡 3、❌ 2**（push / 命名 / 单测 / 日记 / **发表 4/4** / **代码推进（import-root 改名已执行·93 passed）** ✅；clone 路径 / **近邻 discussion 已发** / **领域隔离判据重定义后 1/2** 🟡；引用讨论 / 外审对接 ❌）。较 2026-06-07（✅4/🟡2/❌5）净改善 4 项：发表 ❌→✅、近邻 ❌→🟡、领域隔离 ❌→🟡、**代码推进 🟡→✅（ADR-039 改名执行 / 93 passed）**。**仅剩的 2 个 ❌（引用讨论 / 外审对接）全部需用户真人动作——引用靠时间发酵，外审靠联系真人（SOP 已备）。** 整改见 [d-route-progress-review-2026-06-07](reports/d-route-progress-review-2026-06-07.md) §6 + 外审包 [case-2-external-review-package](methodology/case-2-external-review-package-v0.1.md) + 对接 SOP [external-review-outreach-process](reports/external-review-outreach-process-2026-06-20.md)。
...
### 2.2.4 Sprint P 进度（v0.2 patch + framework v0.2.0 release prep / 1 会话内完成 / 关档）

| Stage | 状态 | 产出 |
|-------|------|------|
| Stage 0 inventory（轻量内嵌 brief §2）| ✅ | brief-template 第 5 次 / 8 项待办清单 + 4 模块 release prep 列表 |
| Stage 1 批 1 — DGF-O-01 (P2) path 改环境变量 | ✅ | 4 helpers `HUADIAN_DATA_DIR` env var > `parents[4]` fallback |
| Stage 1 批 2 — 5 项 template polish | ✅ | brief-template v0.1.2（§1.2 / §3.A+§3.B / §8）+ retro+stop-rules cross-ref + stage-3-review §2.0 选择指南 |
| Stage 1 批 3 — 2 项 code patches | ✅ | DGF-N-01 FIELD_ALIASES 通用机制 + DGF-O-04 inspect.isawaitable() |
| Stage 1 批 4 — v0.2 release prep | ✅ | 4 README §8 v0.2 行 + 2 `__version__` bump 0.1.0→0.2.0 + RELEASE_NOTES_v0.2.md |
...
| **Stage 1.13 dogfood ✅ user local PASSED** | ✅ **PASSED** | list_pending 20/20 + decisions_for_surface 4 surfaces 全部一致 ✓ / 60 pytest 全绿 / **0 Stop Rule 触发** |
| Stage 4 Closeout + Retro | ✅ | stage-4-closeout + sprint-q-retro + sprint-q-residual-debts + STATUS/CHANGELOG |

### 2.2.6 Sprint R 进度（v0.3 patch + methodology/02 v0.1.1 + 5 段元 pattern / 1 会话 ~95 min / 关档）
...
| Stage | 状态 | 产出 |
|-------|------|------|
| Stage 0 inventory + brief 起草 | ✅ | brief-template v0.1.3 第 1 次外部 dogfood |
| Stage 1 批 1 — ADR-030 v0.3 release timing | ✅ | 219 行 / 选项 B / 6/6 触发条件达成 |
| Stage 1 批 2 — methodology/01 v0.1.2 | ✅ | + §10 4 段 cross-ref |
| Stage 1 批 3 — methodology/03 v0.1.2 | ✅ | + §9 4 段 cross-ref |
| Stage 1 批 4 — methodology/04 v0.1.2 | ✅ | + §8 5 段 cross-ref |
| Stage 1.13 + Stage 4 | ✅ | 60/60 in 0.31s + closeout + retro |

### 2.2.8 Sprint T 进度（v0.3.0 release sprint + T-V03-FW-005 Docker compose / 关档）
...
| Stage | 状态 | 产出 |
|-------|------|------|
| Stage 0 + Stage 1 批 1-4 + Stage 1.13 + Stage 4 | ✅ | ADR-031 + methodology/06 v0.1.1 + /07 v0.1 + /00 v0.1.1 + 8 doc 完整 |

### 2.2.10 Sprint V 进度（v0.4 maintenance fold + methodology/02 v0.2 起步 / 1.5 会话 / 关档）

| Stage | 状态 | 产出 |
|-------|------|------|
| Stage 0 inventory + brief 起草 | ✅ | brief-template v0.1.3 第 4 次外部 dogfood (Docs 主导) |
| Stage 1 批 1 — ADR-032 audit_triage retroactive | ✅ | 196 行 / status accepted (retroactive) / 首个 retroactive ADR ⭐ |
| Stage 1 批 2 — chief-architect §工程小细节 v0.3.1 | ✅ | + 第 4 条 commit message hygiene + role-templates v0.3.1 |
| Stage 1 批 3 — brief-template v0.1.4 | ✅ | §2.1 7 子类拆分（Code 3 + Docs 3 + Closeout 1）+ sprint-templates v0.3.1 |
| **会话 1 中场 commit + push (`7c5ce84`)** | ✅ | main `7c5ce84`（3 commits）|
| **Stage 1 批 4 — methodology/02 v0.1.1 → v0.2** ⭐ | ✅ | + §14 Eval Sprint Pattern + §15 Release Sprint Pattern + 重组 §9 (4→6 patterns) / 716 行 |
| Stage 1.13 sanity | ✅ | 60/60 in 0.26s + 5 模块 + ruff/format clean |
| Stage 4 Closeout + Retro | ✅ | stage-4-closeout + sprint-v-retro + sprint-v-residual-debts |

### 2.2.11 Sprint W 进度（methodology v0.2 cycle 持续 / /05 + /07 → v0.2 / 1.5 会话 / 关档）

| Stage | 状态 | 产出 |
|-------|------|------|
| Stage 0 inventory + brief 起草 | ✅ | brief-template **v0.1.4 第 1 次外部 dogfood**（7 子类首验证）|
| **Stage 1 批 1 — methodology/05 v0.1.1 → v0.2** ⭐ | ✅ | + §8 Audit Immutability Pattern (multi-row + surface_snapshot 冻结 first-class) + §7.6 ADR-032 retroactive 引用 / +131 行 / 532 总行（Stop Rule #3 阈 600 内）|
| **Stage 1 批 2 — methodology/07 v0.1 → v0.2** ⭐ | ✅ | + §9 Tooling Pattern for Cross-Stack Abstraction（4 子模式：pglast / Approach B Docker / pre-commit hook / Hybrid Release）+ §8 cross-ref 表更新 / +125 行 / 426 总行（Stop Rule #4 阈 450 内）|
| Stage 1.13 sanity | ✅ | 60/60 in 0.06s + 5 模块 + ruff/format clean |
| Stage 4 Closeout + Retro | ✅ | stage-4-closeout + sprint-w-retro + sprint-w-residual-debts |

### 2.2.12 Sprint X 进度（methodology v0.2 cycle 持续 / /06 + /04 → v0.2 / 1.5 会话 / cycle 过半 ⭐ / 关档）
...
### 2.2.14 Sprint Z 进度（**methodology v0.2 cycle 完成 sprint** ⭐⭐⭐ / /03 → v0.2 / 8/8 = 100% / **ADR-031 #7 触发 / v1.0 评估议程激活** / 1.5 会话 / 关档）

| Stage | 状态 | 产出 |
|-------|------|------|
| Stage 0 inventory + brief 起草 | ✅ | brief-template **v0.1.4 第 4 次外部 dogfood**（单 doc 大工作子类首验证）|
| **Stage 1 (单批) — methodology/03 v0.1.2 → v0.2** ⭐⭐⭐ | ✅ | + §10 Byte-Identical Dogfood Pattern first-class（8 sub-sections：定义 + 4 等级 Dogfood 框架完整化 + 适用条件 + 设计契约 + Sprint N 实证 + 跨域 fork 启示 + vs Self-Test 对比 + 反模式）/ +119 行 / 510 总行（85% 容量）|
| Stage 1.13 sanity | ✅ | 60/60 in 0.11s + 5 模块 + ruff check passed + 84 files format clean |
| Stage 4 Closeout + Retro | ✅ | stage-4-closeout + sprint-z-retro (16th external) + sprint-z-residual-debts |

### 2.2.15 Sprint AA 进度（**Layer 2 维护态首 sprint** / v0.5 maintenance / 清 3 candidates / 1 会话紧凑 / 关档）

| Stage | 状态 | 产出 |
|-------|------|------|
| Stage 0 inventory + brief 起草 | ✅ | brief-template **v0.1.4 第 5 次外部 dogfood**（Docs: cross-ref polish 子类首独立验证）|
| Stage 1 批 1 — T-V05-FW-002 /02 v0.2 → v0.2.1 | ✅ | §14.4 + §15.4 cross-ref polish (引 /06 §8 single source of truth) / +6 行 / 722 总行 |
| Stage 1 批 2 — T-X02-FW-001 /04 v0.2 → v0.2.1 | ✅ | fold §8.6 deferred (per Sprint X 方案 B) / §8.6.1 V9 SelfTest impl 范例 + §8.6.2 反模式 4 条 / +42 行 / 597 总行（99.5% Stop Rule #4 / 紧凑写法成功避触发）|
| Stage 1 批 3 — T-X02-FW-002 brief-template v0.1.5 | ✅ | + §2.5 v0.x 大 bump 重编号 checklist + §2.6 里程碑庆祝节制 / sprint-templates v0.3.1 → v0.3.2 |
| Stage 1.13 sanity | ✅ | 60/60 in 0.06s + 5 模块 + ruff check passed + 84 files format clean |
| Stage 4 Closeout + Retro | ✅ | stage-4-closeout + sprint-aa-retro (17th external) + sprint-aa-residual-debts |

### 2.3 阻塞 / 风险 / 等待项 — **维护态运行中**

- 无阻塞。Sprint AA 关档。**Layer 2 维护态首 sprint 模板验证**。
- 待用户在 local Terminal 执行：`git commit + push`（2 commits 详见 `docs/sprint-logs/sprint-aa/stage-4-closeout-2026-04-30.md` §9）
