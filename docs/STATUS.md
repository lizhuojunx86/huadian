...
### T-P0-024 α — 历史章节证据链回填
- [x] Stage 0a：侦察（β 现状 + 路径分析）
- [x] Stage 1a：代码侦察（load.py evidence 写路径 + 复用原子）
- [x] Stage 1b：backfill_evidence.py 脚本实现 + dry-run 验证
- [x] Stage 1c：β Path A 真实写入（Gate 0-4 协议，V7 52.48%→58.21%）
- [x] Stage 1d：C1 收尾（merge to main + sprint log + STATUS）
- [ ] Stage 2a：Phase A/B diff 分析（v1-r4 重跑预测）
- [ ] Stage 2b：Phase A/B Path B 执行（99 段重跑 extract → evidence 写入）
- [ ] Stage 3：Sprint 收尾（V7 目标 80%+，CHANGELOG，debt 登记）
- **当前状态**：C1 完成，C2 待执行
- **V7 进度**：52.48% → 58.21%（β 天花板达到，Phase A/B 待跑）
...
- 📊 Evidence 覆盖：source_evidences 266 行 / V7 覆盖率 58.21%（C1 β done; target: ≥80% after C2 Phase A/B backfill）
...
| V7 | evidence coverage | active person_names 的 source_evidence_id 覆盖率 | ✅ 58.21% (PASS) | 2026-04-20（T-P0-024 α C1 β backfill） |

**V1-V6 全绿；V7 58.21% PASS（C1 β Path A done, +30 names; 存量 219 行待 C2 Phase A/B 回填→目标 ≥80%）**。
