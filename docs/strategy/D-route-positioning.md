...
我们的护城河构建路径：
1. ~~华典智谱史记案例做扎实（5-10 篇延伸级 demo）~~ **撤销（ADR-038 2026-06-07）**：史记 = 冻结 dogfood 底物（729 persons / 3-4 本纪深度结构化），案例扎实性由 case-2 顶替
...
**主案例**：
- ~~华典智谱史记：再做 5-7 篇典型章节（候选：吕后本纪 / 孝文本纪 / 项羽列传 / 刺客列传 / 货殖列传 / 太史公自序）~~ **撤销（ADR-038 2026-06-07）**：史记冻结为 dogfood 底物；活跃案例 = case-2 中成药提取（数据完整性审计 / 双命题归属见 ADR-041 A2）
- ~~选择标准：覆盖不同体裁（本纪 / 列传 / 自序）+ 不同人物密度 + 能验证不同框架抽象~~（随上行撤销）
...
| 2027-01 (9mo) | 框架代码抽象 v0.1 release（单仓内 `docs/methodology` + `kb_forge/` 双成熟）— **2026-07-16 起冻结为触发制（ADR-041 A3）：出现第 1 个真实外部使用者才解锁；日程锚失效** | ~~L1 工作不被中断~~ → 外部使用信号 |
...
### Layer 3 (案例库):
- ~~华典智谱史记延伸到的章节数（目标 8mo 内 5-10 篇）~~ **撤销（ADR-038）**；替代指标：case-2 学术制品对外可验证进展（SSRN → 外审 → v1.0）
...
| v1.1 | 2026-07-16 | Architect | audit-2026-07-16 黄色批次：§3.3/§5-L3/§10-L3 三处史记旧目标按 ADR-038 划线（此前仅 §6 已改）；§6 2027-01 v0.1 release 锚按 ADR-041 A3 冻结为触发制 |
