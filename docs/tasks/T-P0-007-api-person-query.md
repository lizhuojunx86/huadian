...
### Q-5 sourceEvidenceId 在 Person 上的处理 ✅ 已裁决

> **裁决**：[ADR-009](../decisions/ADR-009-person-source-evidence-traceability.md) — **Option beta（nullable 放宽）**
>
> `Traceable.sourceEvidenceId` 从 `ID!` 改为 `ID`（nullable）。字典种子人物的 `sourceEvidenceId` 返回 null，traceability 由 `provenanceTier`（non-null）+ `evidence_links`（多对多）共同保证。C-2 合规（OR 条件）。
>
> 此变更是系统性的：`books`/`events`/`places` 表同样无 `source_evidence_id` 列，SDL 需对所有 5 个 Traceable implementor 统一放宽。

**SDL 变更（S-0.5 前置步骤，后端工程师执行）**：
...
变更后执行 `pnpm --filter @huadian/api codegen` + `schema:merge` 重生成 TS types。
...
### S-0 架构师评审（开工前）
- [x] 后端工程师提交本文件
- [x] 架构师裁定 Q-1~Q-5（Q-5 → ADR-009 accepted）
- [ ] 用户确认后开工

### S-0.5 SDL nullable 变更（ADR-009 实施，后端工程师，~0.1 天）
- [ ] 6 个 SDL 文件 `sourceEvidenceId: ID!` → `ID`（见 Q-5 裁决表格）
- [ ] `pnpm --filter @huadian/api codegen` + `schema:merge`
...
| 风险 | 缓解 |
|------|------|
| ~~Q-5 sourceEvidenceId 字段在 persons 表缺失~~ | ✅ ADR-009 裁决：Traceable.sourceEvidenceId 改为 nullable；S-0.5 实施 SDL 变更 |
...
- [x] **C-2** Traceable.sourceEvidenceId 保证溯源（Q-5 → ADR-009：nullable + provenanceTier 满足 C-2 OR 条件）
...
- 2026-04-17 v2：Q-5 裁决落地（ADR-009 accepted — Option beta nullable 放宽）；新增 S-0.5 SDL 变更子任务；风险表 + 宪法清单更新
