...
_CONFIG_YML = (
    Path(__file__).resolve().parents[2] / "config" / "traceguard_policy.yml"
)
...
def test_malformed_top_level_raises() -> None:
    with pytest.raises(PolicyConfigError):
        _ = ActionPolicy.from_dict(
            {"by_severity": {}}
        )  # empty by_severity is rejected


def test_unknown_severity_raises() -> None:
    with pytest.raises(PolicyConfigError):
        _ = ActionPolicy.from_dict(
            {"by_severity": {"SEVERE": ["retry"]}}
        )
...
def test_unknown_action_raises() -> None:
    with pytest.raises(PolicyConfigError):
        _ = ActionPolicy.from_dict(
            {"by_severity": {"critical": ["warn_only"]}}
        )
...
def test_by_step_override_beats_by_severity() -> None:
    ...
    assert p.resolve(
        ...
        attempt=2,
        tg_action="retry",
    ) == "human_queue"  # would be `degrade` under by_severity
