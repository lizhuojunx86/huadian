...
description: >
  ...
  v1-r5: +官衔爵位+人名复合识别(T-P2-004, Sprint F).
created: 2026-04-18
revised: 2026-04-25
---
...
## 官衔/爵位 + 人名 复合识别（v1-r5 新增）

战国秦汉文献中，人物常以"**官衔/爵位 + 名**"的复合形式出现（如"五大夫礼""蜀相壮""蜀守若""左庶长商鞅"）。识别规则：

### 基本原则

当原文出现"官衔/爵位 + 人名"复合称谓时：
1. **完整的"官衔+名"作为 surface_form**（如"五大夫礼"，而非截断为仅"礼"）
2. **name_zh 使用完整形式或最通行的称谓**（如 name_zh="蜀相壮" 而非 "壮"）
3. 如果同一人在段落中还以裸名出现（如段落中既有"五大夫礼"又有"礼"），裸名也应作为 surface_form，但优先级低于完整形式

### 常见官衔/爵位模式

秦制二十等爵及常见官职：
- 爵位：公士、上造、簪袅、不更、大夫、官大夫、公大夫、公乘、五大夫、左庶长、右庶长、左更、中更、右更、少上造、大上造/大良造、驷车庶长、关内侯、彻侯
- 官职：相、丞相、将军、御史、太史、太傅、太尉、守（郡守）、令、尉、使、客卿、庶长
- 地方官：蜀守、蜀相、陇西守、北地守

当上述词紧接人名时，整体作为一个 surface_form。
...
```json
{
  "name_zh": "蜀相壮",
  "surface_forms": [
    {"text": "蜀相壮", "name_type": "primary"}
  ]
}
```
**不要**截断为 name_zh="壮"，surface_forms=[{"text":"壮"}]
...
如果"官衔+名"中的名本身已是知名人物的通行称谓，则 name_zh 使用通行称谓，"官衔+名"作为 nickname：
...
```json
{
  "name_zh": "商鞅",
  "surface_forms": [
    {"text": "商鞅", "name_type": "primary"},
    {"text": "左庶长商鞅", "name_type": "nickname"}
  ]
}
```

判断标准：该人物是否有跨段/跨章反复出现的独立通行名。若仅在本段以"官衔+名"出现一次且无其他称谓上下文，则以完整形式为 name_zh。
