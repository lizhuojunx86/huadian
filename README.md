...
**简短版**：一个用 AI agent 团队构建知识库的工程框架 + 一个真实的实现案例（《史记》）。具体交付三样东西：可直接 `pytest` 验证的纯 Python 框架核心（`kb_forge/`）、复制即用的 Sprint / 角色治理模板、以及完整公开的工程过程记录（34 ADRs + Sprint A-AA 全部日志）。
...
`kb_forge/`（发行名 **kb-forge**）核心是 **pure-Python（stdlib only）** — 不需要数据库、不需要 Docker：
...
- 读 [`kb_forge/README.md`](kb_forge/README.md) — 5 个模块各是什么
...
- **`kb_forge/` 5 模块就位（v0.3.0）** — identity_resolver / invariant_scaffold / audit_triage 三个 Python 包（各带 tests + examples，全套 **93 tests passed**）+ sprint-templates v0.3.2 + role-templates v0.3.1
...
## 框架抽象成果（Layer 1 / `kb_forge/` = kb-forge）
...
| 模块 | 形态 | 抽象自 |
|------|-----|-------|
| [`kb_forge/identity_resolver/`](kb_forge/identity_resolver/) | Python 包 | R1-R6 跨 chunk 实体消解 + 可插拔 `GUARD_CHAINS` |
| [`kb_forge/invariant_scaffold/`](kb_forge/invariant_scaffold/) | Python 包 | V1-V11 形式化质量约束（invariant pattern）|
| [`kb_forge/audit_triage/`](kb_forge/audit_triage/) | Python 包 | pending-review 队列 + 不可变决策审计 + hint banner |
| [`kb_forge/sprint-templates/`](kb_forge/sprint-templates/) | 文档模板 v0.3.2 | Sprint / Stage / Gate 工作流 + Stop Rules |
| [`kb_forge/role-templates/`](kb_forge/role-templates/) | 文档模板 v0.3.1 | 10 个领域中立 agent 角色定义 + tagged-sessions protocol |
...
- Sprint L 用 kb_forge/sprint-templates/ 给自己收档：覆盖度 90%
- Sprint M 用 kb_forge/role-templates/ 回审 Sprint K 5 角色协同实战：覆盖度 **99.2%**
...
**诚实状态**：领域无关 LOC 比例当前实测 ~56-62%（目标 ≥ 70%，`examples/` 仍偏重）— 完整判据表见 [`kb_forge/README.md`](kb_forge/README.md)。

### 下一步（roadmap）

import root 改名 `kb_forge.*` → 领域无关 LOC 比例提升至 ≥ 70% → `pip install -e` 可安装打包（详见 [ADR-037](docs/decisions/ADR-037-framework-package-naming.md) + [`kb_forge/README.md`](kb_forge/README.md)）。
...
```
kb_forge/             → **Layer 1 框架抽象产出 (kb-forge v0.3.0)**（跨领域 KE 项目复用）
...
docs/
  ...
  └── STATUS.md                 → 实时状态板
.claude/agents/        → 10 个 agent 角色定义（华典智谱实例 / kb_forge/role-templates 的具体 instantiation）
```
...
**框架抽象（Layer 1，可立即复用）**：
- [kb_forge/](kb_forge/) — kb-forge v0.3.0 总览（5 模块 + Quick start + 完成判据）
- [kb_forge/sprint-templates/](kb_forge/sprint-templates/) — Sprint 治理模板 v0.3.2
- [kb_forge/role-templates/](kb_forge/role-templates/) — 多角色协作模板 v0.3.1
