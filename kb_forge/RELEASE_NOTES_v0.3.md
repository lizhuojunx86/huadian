...
- **新模块**：kb_forge/audit_triage/ v0.1（Sprint Q）→ v0.3.0 同步 release
...
|------|---------|-----------|------|---------|
| `kb_forge/sprint-templates/` | v0.2.0 | **v0.3.0** | sprint governance 模板 | brief-template v0.1.3 (3 类估时表 / Sprint R T-V03-FW-003) |
| `kb_forge/role-templates/` | v0.2.0 (+v0.2.1 patch) | **v0.3.0** | 多角色协作模板 | chief-architect §工程小细节 (Sprint R T-V03-FW-004) |
| `kb_forge/identity_resolver/` | v0.2.0 | **v0.3.0** | 身份解析框架 | README §2.5 公共 API 速查 (Sprint R T-V03-FW-001) + 33 pytest tests (Sprint Q) |
| `kb_forge/invariant_scaffold/` | v0.2.0 | **v0.3.0** | 不变量校验框架 | 27 pytest tests (Sprint Q) / 内容无 ABI 变化 |
| `kb_forge/audit_triage/` | v0.1.0 (Sprint Q first abstraction) | **v0.3.0** | 审计 / triage workflow 框架 | 跳跃式 bump 0.1.0 → 0.3.0 对齐统一版本号；内容自 v0.1 起无 ABI 变化 |
...
| T-V03-FW-006 | R | pre-commit hook | scripts/check-audit-triage-sync.sh + .pre-commit-config.yaml 加 services↔kb_forge/audit_triage sync warning hook |
...
- **激活 services↔kb_forge/audit_triage sync hook**（如果你 fork audit_triage 到自己的领域）
...
- [x] 顶层 `kb_forge/RELEASE_NOTES_v0.3.md`（本文件 / Sprint T 批 3）
