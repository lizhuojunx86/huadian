...
被 guard 拦截的 merge → `BlockedMerge` 数据结构 → 案例方 `MergeApplier.write_blocked_review()` 写入自己的 triage 表（华典：pending_merge_reviews）。Domain Expert 走 Triage UI workflow 决策（参见 `kb_forge/role-templates/` 文档）。
...
> 本文件描述的概念是 kb_forge/identity_resolver/ 的**理论基础**。具体使用方法见 README.md / 跨领域 mapping 见 cross-domain-mapping.md。
