# kb_forge/identity_resolver — Identity Resolution Framework

> Status: **v0.3.0 (Sprint T release / 2026-04-30)**
> First abstraction: Sprint N Stage 1, 2026-04-30
> Latest patch: Sprint R T-V03-FW-001 (README §2.5 公共 API 速查 / 38 export 7 类)
> Date: 2026-04-30
...
**核心信念**（与 kb_forge/sprint-templates + kb_forge/role-templates 一脉相承）：
...
下游 fork 或 import 本框架时，**只用 `from kb_forge.identity_resolver import …`** 这一层；
不要直接 import `kb_forge.identity_resolver.rules` / `.guards` / `.resolve` 等子模块（除非你在写 framework 本身或 examples/）。
...
# → 直接 import: from huadian.kb_forge.identity_resolver import ...

# 选项 B: 复制框架核心到你自己的项目
cp -r huadian/kb_forge/identity_resolver /path/to/your-project/your_pkg/
```
...
from kb_forge.identity_resolver import EntitySnapshot
...
from kb_forge.identity_resolver import BlockedMerge, HypothesisProposal
...
```python
from kb_forge.identity_resolver import (
...
```python
from kb_forge.identity_resolver import apply_merges
...
- ❌ **不要直接修改 kb_forge/identity_resolver/ 用于自己案例** — fork / cp 到自己项目，改 `examples/your_domain/`
...
| **v0.3.0** | **2026-04-30** | **Sprint T v0.3 release** | T-V03-FW-001 (Sprint R): README §2.5 加"公共 API 速查"段（38 个 export 按 7 类分组 + fork 决策树）；与 framework v0.3 release 同步发布 |

---

> 本框架是 AKE 框架 Layer 1 的**第三刀**（代码层第一刀）。
> 第一刀：kb_forge/sprint-templates/（Sprint L）
> 第二刀：kb_forge/role-templates/（Sprint M）
> 第三刀：kb_forge/identity_resolver/（Sprint N）
