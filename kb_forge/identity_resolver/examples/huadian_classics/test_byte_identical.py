...
Runs two dry-run passes against the same HuaDian Postgres database:
    ...
    B. kb_forge.identity_resolver.resolve_identities()
       + kb_forge.identity_resolver.examples.huadian_classics adapters
...
Usage:
    cd <project_root>
    uv run python -m kb_forge.identity_resolver.examples.huadian_classics.test_byte_identical
...
from kb_forge.identity_resolver.examples.huadian_classics import (
    di_honorific_hint,
    dictionary_loaders,
    guard_chains,
...
async def main() -> int:
    ...
    if identical:
        ...
        with open("/tmp/sprint-n-dogfood-kb_forge.json", "w") as f:
