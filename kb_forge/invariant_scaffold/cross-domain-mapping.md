...
> Source: kb_forge/role-templates/cross-domain-mapping.md + kb_forge/identity_resolver/cross-domain-mapping.md 同款 6 领域 + 5 pattern 适配
...
```
kb_forge/invariant_scaffold/examples/legal/
  __init__.py
...
from kb_forge.invariant_scaffold import LowerBoundInvariant
...
```python
from kb_forge.invariant_scaffold.examples.legal.runner_setup import build_legal_runner
from kb_forge.invariant_scaffold.examples.legal.db_port import LegalAsyncpgPort
...
如果你用 `kb_forge/invariant_scaffold/` 启动了一个**本表未覆盖的领域**（如金融文档 / 学术论文 / 代码 corpus / etc），欢迎：
...
> 本文件继承 kb_forge/role-templates/cross-domain-mapping.md + kb_forge/identity_resolver/cross-domain-mapping.md 同款 6 领域结构，但聚焦 invariant pattern 适配。三文档配合使用：role-templates 是 who（谁来做）/ identity_resolver 是 how-to-resolve（怎么消歧）/ invariant_scaffold 是 how-to-verify（怎么验证）。
