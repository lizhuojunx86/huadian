# kb_forge/sprint-templates — Sprint Governance Templates
...
```bash
# 1. 复制本目录到你的项目
cp -r /path/to/huadian/kb_forge/sprint-templates /path/to/your-project/sprint-templates
...
| **v0.3.2** | **2026-04-30** | **Sprint AA 批 3 v0.5 maintenance patch** | brief-template v0.1.4 → v0.1.5（加 §2.5 v0.x 大 bump 重编号 checklist + §2.6 里程碑庆祝节制 / 6 次 dogfood 验证 v0.1.4 7 子类估时表已收敛 ≤ 5% target / 不动）— T-X02-FW-002 落地 / methodology v0.2 cycle 完成后首个 v0.5 maintenance sprint 输出 |

---

> 本套模板是 AKE 框架 Layer 1 的第一刀。后续 Layer 1 抽象（multi-role coordination / identity resolver / invariants / audit / etc）将陆续在 `kb_forge/` 下出现。
