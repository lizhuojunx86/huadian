...
|------|---------|-----------|------|--------|
| `kb_forge/sprint-templates/` | v0.1.1 | **v0.2.0** | sprint governance 模板 | L (第一刀) |
| `kb_forge/role-templates/` | v0.1.1 | **v0.2.0** | 多角色协作模板 | M (第二刀) |
| `kb_forge/identity_resolver/` | v0.1.0 | **v0.2.0** | 身份解析框架 | N (第三刀) |
| `kb_forge/invariant_scaffold/` | v0.1.0 | **v0.2.0** | 不变量校验框架 | O (第四刀) |
...
修法：
- `kb_forge/identity_resolver/examples/huadian_classics/` 3 处 path helper（`_default_dict_dir` / `_default_periods_path` / `_default_states_path`）
- `kb_forge/invariant_scaffold/examples/huadian_classics/invariants_slug.py` 1 处（拆出 `_default_tier_s_path`）
...
- [x] 顶层 `kb_forge/RELEASE_NOTES_v0.2.md`（本文件）
