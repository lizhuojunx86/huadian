...
> Date: 2026-04-29
> License: CC BY 4.0
> Source: `kb_forge/sprint-templates/README.md` §4.1 (4 列 mapping) + `docs/methodology/01-role-design-pattern.md` §2 + Sprint M Stage 0 inventory §6
...
其他 9 个角色（Architect / PE / BE / FE / PM / Designer / QA / DevOps / Analyst）跨领域不需要重命名，只需改少量路径名 / 技术栈名（参见 `kb_forge/role-templates/README.md` §4）。
...
参见 `.claude/agents/historian.md`（华典智谱原始版本）+ `kb_forge/role-templates/domain-expert.md`（领域无关模板）。
...
→ Stop Rule 阈值（参见 `kb_forge/sprint-templates/stop-rules-catalog.md` §1 cost 类）必须按领域调整。
...
如果你用 `kb_forge/role-templates/` 启动了一个**本表未覆盖的领域**（如金融文档 / 学术论文 / 代码 corpus / 政策文件 / etc），欢迎：
...
> 与 `kb_forge/role-templates/domain-expert.md` 配合使用。
