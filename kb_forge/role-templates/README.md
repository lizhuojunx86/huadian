# kb_forge/role-templates — Multi-Role Coordination Templates
...
```bash
# 1. 复制本目录到你的项目
cp -r /path/to/huadian/kb_forge/role-templates /path/to/your-project/.claude/agents
...
> 本套模板是 AKE 框架 Layer 1 的第二刀。第一刀是 `kb_forge/sprint-templates/`（Sprint L Stage 1 完成）。
> 两套模板配合使用，构成 kb_forge/ 下"治理类"双模块——sprint workflow + role coordination。
